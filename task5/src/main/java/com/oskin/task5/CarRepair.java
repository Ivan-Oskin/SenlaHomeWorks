...
public class CarRepair {
    ...
    private static CarRepair instance;
    ...
    private <T extends Nameable> boolean delete(String name, ArrayList<T> list) {
    ...
    boolean deleteMaster(String name) {
        return delete(name, masters);
    }
    ...
    boolean deletePlace(String name) {
        return delete(name, garage);
    }
    ...
    boolean deleteOrder(String name) {
        return delete(name, orders);
    }

    boolean completeOrder(String name) {
        int i = findByName(name, orders);
        if (i == -1) {
            return false;
        } else {
    ...
    boolean cancelOrder(String name) {
        int i = findByName(name, orders);
        if (i == -1) {
            return false;
        } else {
    ...
    public Place findPlace(String name) {
        int count = findByName(name, garage);
        if (count == -1) {
            return null;
        } else {
    ...
    public boolean setOrderToMaster(String nameMaster, String nameOrder) {
        int i = findByName(nameMaster, masters);
        int j = findByName(nameOrder, orders);
        if (i >= 0 && j >= 0) {
            Master master = (Master) masters.get(i);
            master.addOrder((Order) orders.get(j));
            return true;
        } else {
            return false;
        }
